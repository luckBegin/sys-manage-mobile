...
export class PromotionBookComponent implements OnInit {
	...
	public cancelBook( item: any): void {
		this.modal.alert('取消', '确认取消该安排?', [
			{ text: '取消'},
			{ text: '确定', onPress: () => {
				this.cancelReplacement(item) ;
			}},
		]);
	}

	private cancelReplacement( item ): void {
		this.toast.loading('处理中...', 0) ;
		this.service.cancelBook({ id: item.id })
			.subscribe( ( res: RESPONSE ) => {
				this.msg.success('安排已取消') ;
				this.bookListQueryModel = new BookListQueryModel();
				this.getList(true) ;
			});
	}
